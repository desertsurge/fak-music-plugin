...
Fak Music 是一个 Chrome/Edge Manifest V3 音乐扩展开发样例。安装后可以浏览内置的网络电台和开放授权单曲；目录在后台尝试更新，远程服务不可用时继续使用最后一次可用的本地数据。当前仓库中的目录地址和音频地址仍是开发样例，不代表已接入生产音乐供应商。

## 特性

- 内置电台和开放授权单曲，首次打开无需配置；配置远程目录后启动时后台会立即刷新
...
开发页面默认地址为 `http://127.0.0.1:5173/`。扩展构建：
...
上面的 GitHub Raw 地址只是配置示例，当前未作为默认地址启用；fallback 中的单曲地址仍是开发样例，尚未完成持续可用性审核。它们用于演示目录协议、缓存回退和播放错误恢复，并不承诺真实音频长期可用，也不代表已完成真实内容源的审核或维护。目录结构、自动更新设想和许可证策略见 [`docs/music-presets-auto-update-plan.md`](docs/music-presets-auto-update-plan.md)。
...
本轮验证了构建、自动化测试、Popup/UI 交互，以及 Edge 未打包扩展中的部分真实音频播放和加载超时跳过；Internet Archive 示例单曲和远程目录当前仍存在外部不可用或未配置情况。没有把所有开发样例地址长期可用或浏览器扩展商店发布当作已完成事项。
